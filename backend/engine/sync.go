...
// SyncOptions allows callers to override default sync behaviour.
type SyncOptions struct {
	SinceOverride      *time.Time // Override the default since timestamp
	ConversationLimit  int        // Override the default conversation limit (0 = use default)
}
...
// SyncChannel syncs a single channel with default options (used by scheduler).
func (s *SyncEngine) SyncChannel(ctx context.Context, channel models.Channel) error {
	return s.SyncChannelWithOptions(ctx, channel, SyncOptions{})
}

// SyncChannelWithOptions syncs a single channel with configurable options.
func (s *SyncEngine) SyncChannelWithOptions(ctx context.Context, channel models.Channel, opts SyncOptions) error {
	...
	// Determine since — use override, last_sync_at, or default to 7 days ago
	var since time.Time
	if opts.SinceOverride != nil {
		since = *opts.SinceOverride
		log.Printf("[sync] channel %s: using since override %s", channel.Name, since.Format(time.RFC3339))
	} else {
	...
	// Conversation limit — use override or default 100
	convLimit := 100
	if opts.ConversationLimit > 0 {
		convLimit = opts.ConversationLimit
	}

	// Fetch recent conversations
	conversations, err := adapter.FetchRecentConversations(ctx, since, convLimit)
	if err != nil {
