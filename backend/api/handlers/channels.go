...
func SyncChannelNow(c *gin.Context) {
	...
	var syncOpts engine.SyncOptions
	syncMode := c.Query("mode")
	sinceParam := c.Query("since")

	if syncMode == "full" {
		// Full sync: use zero time (all history), higher conversation limit
		zeroTime := time.Time{}
		syncOpts.SinceOverride = &zeroTime
		syncOpts.ConversationLimit = 500
	} else if sinceParam != "" {
		// Parse since date (supports YYYY-MM-DD or RFC3339)
		parsed, err := time.Parse("2006-01-02", sinceParam)
		if err != nil {
			parsed, err = time.Parse(time.RFC3339, sinceParam)
		}
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "invalid_since_format", "detail": "Use YYYY-MM-DD or RFC3339"})
			return
		}
		syncOpts.SinceOverride = &parsed
		syncOpts.ConversationLimit = 500
	}

	// Use longer timeout for full/historical sync
	syncTimeout := 10 * time.Minute
	if syncOpts.SinceOverride != nil {
	...
	go func() {
		...
		defer cancel()
		if err := syncEng.SyncChannelWithOptions(ctx, channel, syncOpts); err != nil {
