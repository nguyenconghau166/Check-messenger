  ...
  <div v-if="channel">
    ...
    <v-dialog v-model="syncDateDialog" max-width="350">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn variant="text" @click="syncDateDialog = false">{{ $t('cancel') || 'Hủy' }}</v-btn>
...
async function doSync(opts?: { mode?: string; since?: string }) {
  syncing.value = true
  syncResult.value = null
  const maxPollTime = opts?.mode === 'full' || opts?.since ? 10 * 60 : 3 * 60 // 10 min for full, 3 min for regular
  try {
    await channelStore.syncChannel(tenantId.value, channelId.value, opts)
    // Poll channel status until sync completes
    let pollAttempts = 0
    const maxPollAttempts = Math.floor(maxPollTime / 3)
...
async function doSyncFull() {
  ...
  await doSync({ mode: 'full' })
}

function openSyncFromDate() {
  syncDateValue.value = ''
...
async function confirmSyncFromDate() {
  ...
  if (syncDateValue.value) {
    await doSync({ since: syncDateValue.value })
  }
}
