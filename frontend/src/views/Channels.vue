<template>
  <div>
    ...
    <!-- Sync from date dialog -->
    <v-dialog v-model="syncDateDialog" max-width="350">
      <v-card>
        <v-card-title>{{ $t('sync_from_date') }}</v-card-title>
        <v-card-text>
          <v-text-field v-model="syncDateValue" type="date" density="compact" />
        </v-card-text>
        <v-card-actions>
          <v-spacer />
          <v-btn variant="text" @click="syncDateDialog = false">{{ $t('cancel') }}</v-btn>
          <v-btn color="primary" :disabled="!syncDateValue" @click="confirmSyncFromDate">{{ $t('sync_now') }}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
const syncing = ref('')
const reauthing = ref('')
const syncDateDialog = ref(false)
const syncDateValue = ref('')
const syncDateChannelId = ref('')
...
async function syncNow(channelId: string, opts?: { mode?: string; since?: string }) {
  syncing.value = channelId
  try {
    const data = await channelStore.syncChannel(tenantId.value, channelId, opts)
...
async function syncFull(channelId: string) {
  if (!confirm(t('sync_full_confirm'))) return
  await syncNow(channelId, { mode: 'full' })
}

function openSyncFromDate(channelId: string) {
  syncDateChannelId.value = channelId
  syncDateValue.value = ''
  syncDateDialog.value = true
}

async function confirmSyncFromDate() {
  syncDateDialog.value = false
  if (syncDateValue.value && syncDateChannelId.value) {
    await syncNow(syncDateChannelId.value, { since: syncDateValue.value })
  }
}
