...
export const useChannelStore = defineStore('channels', () => {
  ...
  async function syncChannel(tenantId: string, channelId: string, opts?: { mode?: string; since?: string }) {
    const params: Record<string, string> = {}
    if (opts?.mode) params.mode = opts.mode
    if (opts?.since) params.since = opts.since
    const { data } = await api.post(`/tenants/${tenantId}/channels/${channelId}/sync`, null, { params })
    return data
  }
